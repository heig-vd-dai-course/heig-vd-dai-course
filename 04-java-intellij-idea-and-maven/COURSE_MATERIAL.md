...
If you are using Linux (WSL included) or macOS, you can then run Maven with the
following command:
...
> following: **Right-click on the project** > **Maven** > **Generate Sources and
> Update Folders**. This will regenerate the sources and update the folders so
> that IntelliJ IDEA can be aware of the new dependencies downloaded by Maven.
...
TODO: Add explanation about the `original` file and the
...
We consider this as **bad practice** as it pollutes the comprehension of the
codebase. You should **only ignore files that are specific to your project**,
the tools you are using and the environment you are working in.
